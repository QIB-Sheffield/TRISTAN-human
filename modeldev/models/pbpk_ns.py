...
class Model(lib.FitFunc):
    ...
    def pars(self):
        return [
            ...
            ['CO', "Cardiac output", 100.0, "mL/sec", 0, np.inf, False, 3], # 6 L/min = 100 mL/sec
            ['T_lh', "Heart & lung mean transit time", 8.0, "sec", 0, 30,False, 3],
            ['Tp_o', "Organs mean transit time", 20.0, "sec", 0, 60, False, 3],
            ['E_o', "Extracellular extraction fraction", 0.15, "", 0, 0.50, False, 4],
            ['Te_o', "Extracellular mean transit time", 120.0, "sec", 0, 800.0, False, 3],
            #['E_b',"Body extraction fraction", 0.05,"", 0.01, 0.15, False, 4],
            # Liver model
            ['R10l', "Baseline R1 (liver)", lib.R1_liver(), "1/sec", 0, np.inf, False, 6],
            ['T_g', "Gut mean transit time", 8.0, "sec", 0, 30, False, 6],
            ['FF_l', "Liver flow fraction", 0.25, "", 0.01, 1, False, 6],
            ['Kve',"Liver venous excretion rate", 1/20,"1/sec", 1/60.0, 1/5.0, False, 6],
            ...
            ['Kvp',"Kidney venous excretion rate", 1/8,"1/sec", 1/20.0, 1/2.0, True, 6],
            ['FF_k', "Kidney flow fraction", 0.1, "", 0.01, 1, True, 6],
            ['F_ki',"Kidney filtration fraction", 0.10, "", 0.001, 0.999, True, 6],
            ['F_kf',"Kidney filtration fraction", 0.10, "", 0.001, 0.999, True, 6],
            ['ut0', "Tubular velocity", 1/100., '1/sec', 0, 1/10, True, 9],
            ['ut1', "Tubular velocity", 1/100., '1/sec', 0, 1/10, True, 9],
            ['ut2', "Tubular velocity", 1/100., '1/sec', 0, 1/10, True, 9],
            ['dr', "Diffusion rate", 1e-4, '1/sec', 0, 10, True, 6],
    ...
    def export_pars(self, export_pars):
        ...
        export_pars.loc['Tc'] = ["Mean circulation time", p.Tp_o+p.T_lh, 'sec'] 
        # Liver
        # Convert to conventional units
        ...
        export_pars.loc['F_ki', ['value', 'unit']] = [100*p.F_ki, '%']
        export_pars.loc['F_kf', ['value', 'unit']] = [100*p.F_kf, '%']
        # Add derived
        F_k = (p.F_ki + p.F_kf)/2
        E_k = F_k/(1+F_k)
        export_pars.loc['Tv'] = ["Vascular transit time", np.divide(1, p.Kvp), 'sec']
        ...
        export_pars.loc['Fl'] = ['Liver blood flow', 60*p.CO*p.FF_l, 'mL/min']
        export_pars.loc['Pl'] = ['Liver perfusion', 6000*p.CO*p.FF_l/self.liver_volume, 'mL/min/100mL']
        export_pars.loc['vel'] = ['Liver extracellular volume', 100*veL, 'mL/100mL']
        export_pars.loc['CL_l'] = ['Liver plasma clearance', 60*p.E_l*p.CO*p.FF_l*self.Hct, 'mL/min']
        ...
        Kp = p.Kvp/(1-E_k)
        vpK = p.CO*p.FF_l*self.Hct/Kp/self.kidney_volume
        export_pars.loc['Fk'] = ['Kidney blood flow', 60*p.CO*p.FF_k, 'mL/min']
        export_pars.loc['Pk'] = ['Kidney perfusion', 6000*p.CO*p.FF_l/self.kidney_volume, 'mL/min/100mL']
        export_pars.loc['GFR'] = ['Glomerular filtration rate', 60*p.CO*p.FF_l*E_k, 'mL/min']
        export_pars.loc['vpK'] = ['Kidney plasma volume', 100*vpK, 'mL/100mL']
