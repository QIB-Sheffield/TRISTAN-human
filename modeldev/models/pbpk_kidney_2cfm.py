...
class Model(lib.FitFunc):
    ...
    def predict_R1(self):
        ...
        Np = dcmri.res_nscomp(t, J_kidneys, p.Kp)
        Nt = dcmri.res_nscomp(t, p.E_k*p.Kp*Np, p.Kt)
    ...
    def pars(self):
        return [
            ...
            ['Kp',"Kidney plasma excretion rate", 1/5,"1/sec", 1/15.0, 1/1.0, True, 6],
            ['FF_k', "Kidney flow fraction", 0.1, "", 0.01, 1, True, 6],
            ['E_k',"Kidney extraction fraction", 0.10, "", 0.001, 0.2, True, 6],
            ['Kt', "Tubular excretion rate", 1/120, '1/sec', 1/300., np.inf, True, 6],
        ]
    ...
    def export_pars(self, export_pars):
        ...
        # Add derived
        export_pars.loc['Tp'] = ["Plasma transit time", np.divide(1, p.Kp), 'sec']
