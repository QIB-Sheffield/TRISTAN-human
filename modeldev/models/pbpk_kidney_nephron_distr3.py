...
class Model(lib.FitFunc):
    ...
    def predict_R1(self):
        ...
        Jt = E_k*Kp*Np
        Nt0 = dcmri.res_plug(t, p.ff*Jt, p.Tt)
        Nt1 = dcmri.res_plug(t, p.ff1*(1-p.ff)*Jt, p.Tt*p.ft1)
        Nt2 = dcmri.res_plug(t, (1-p.ff1)*(1-p.ff)*Jt, p.Tt*p.ft2)
        Nt = Nt0+Nt1+Nt2
    ...
    def pars(self):
        return [
            ...
            ['Tt', "Tubular mean transit time", 1.3*60, 'sec', 0, np.inf, True, 9],
            ['ft1', "Tubular transit time dispersion 1", 1.1, '', 1, np.inf, True, 9],
            ['ft2', "Tubular transit time dispersion 2", 0.9, '', 0, 1, True, 9],
            ['ff', "ff", 0.9, '', 0, 1, True, 9],
            ['ff1', "ff", 0.5, '', 0, 1, True, 9],
        ]
    ...
    def export_pars(self, export_pars):
        ...
        # export_pars.loc['Tt0', ['value', 'unit']] = [p.Tt0/60, 'min']
        # export_pars.loc['Tt1', ['value', 'unit']] = [p.Tt1/60, 'min']
        # export_pars.loc['fn0', ['value', 'unit']] = [100*p.fn0, '%']
        # Add derived
