...
class Model(lib.FitFunc):
    ...
    def _predict_R1(self, Ji): # decoupled
        p = self.p.value
        t = self.t()
        ...
        Ke = p.Kve/(1-E_l)
        # Aorta
        J_aorta = dcmri.prop_simple_body(t, Ji,
            p.T_lh,
            p.E_o, p.Tp_o, p.Te_o,
            p.FF_l*E_l + (1-p.FF_l)*p.E_k, 
    ...
    def predict_R1(self, Ji):
        ...
        E_l = dcmri.lin(t, [p.E_li, p.E_lf])
        Kbh = dcmri.lin(t, [p.Kbh_i, p.Kbh_f])
        Ke = p.Kve/(1-E_l) # < 0.15/0.85 < 1/dt 
        # Aorta
        J_aorta = dcmri.prop_body_liver(t, Ji,
            p.T_lh,
            p.E_o, p.Tp_o, p.Te_o,
            p.T_g, p.FF_l, E_l, Ke,
            p.E_k, 
    ...
    def export_pars(self, export_pars):
        ...
        export_pars.loc['CL_l'] = ['Liver plasma clearance', 60*E_l*p.CO*p.FF_l*self.Hct, 'mL/min']
        export_pars.loc['E_lb'] = ['Liver whole body extraction fraction', 100*E_l*p.FF_l, '%']
        export_pars.loc['k_he'] = ["Hepatocellular tissue uptake rate", 6000*Khe*veL, 'mL/min/100mL'] 
        export_pars.loc['k_bh'] = ["Hepatocellular excretion rate", 6000*Kbh*(1-veL), 'mL/min/100mL']         
        # Mixed derived (body)
        export_pars.loc['E_b'] = ['Whole body extraction fraction', 100*Eb, '%']
        export_pars.loc['C_l'] = ['Liver contribution', 100*E_l*p.FF_l/Eb, '%']
        export_pars.loc['C_k'] = ['Kidney contribution', 100*p.E_k*(1-p.FF_l)/Eb, '%']
        return export_pars
