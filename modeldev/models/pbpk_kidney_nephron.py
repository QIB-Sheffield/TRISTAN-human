...
class Model(lib.FitFunc):
    ...
    def predict_R1(self):
        ...
        #Ti = [p.Ti0, p.Ti1, p.Ti2]
        #Ei = [p.Ei0, p.Ei1, p.Ei2]
        J_kidneys = p.FF_k*self.J_aorta
        J_kidneys = dcmri.prop_plug(t, J_kidneys, p.Ta)
        if p.Tv==0:
            Np = 0*J_kidneys
            Nt = dcmri.conc_neph(t, E_k*J_kidneys, vel, dif, reabs, n=50)
        else:
            Kvp = 1/p.Tv
            Kp = Kvp/(1-E_k)
            ...
            #Nt = dcmri.conc_nephi(t, E_k*Kp*Np, vel, dif, reabs, Ti, Ei, n=50)
    ...
    def pars(self):
        return [
            ...
            # ['Ti0', "Interstitial MTT", 120, 'sec', 30, np.inf, True, 9],
            # ['Ti1', "Interstitial MTT", 120, 'sec', 30, np.inf, True, 9],
            # ['Ti2', "Interstitial MTT", 120, 'sec', 30, np.inf, True, 9],
            # ['Ei0', "Interstitial extraction fraction", 0.00, '', 0.0, 0.2, True, 9],
            # ['Ei1', "Interstitial extraction fraction", 0.00, '', 0.0, 0.2, True, 9],
            # ['Ei2', "Interstitial extraction fraction", 0.00, '', 0.0, 0.2, True, 9],
        ]
