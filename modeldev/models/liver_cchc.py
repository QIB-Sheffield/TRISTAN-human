...
class TwoShotTwoScan(CurveFit):
    ...
    def R1(self): # slower but more diagnostics
        ...
        ca = dcmri.propagate_dd(t, self.aorta.cb, p.Ta_del, p.Ta_disp)
        # Propagate through the gut
        cv = dcmri.propagate_dd(t, self.aorta.cb, p.Tg_del, p.Tg_disp)
        # Mix at the inlet
        cin = p.AFF*ca + (1-p.AFF)*cv
        # Propagate through the extracellular space
        self.Ce = dcmri.res_comp(t, p.fb*cin, np.divide(1,p.Kve+p.Khe))
        # Propagate through the hepatocytes
        self.Ch = dcmri.ures_chain(t[1]-t[0], p.Khe*self.Ce, np.divide(1,p.Kbh), p.Dh)
        # Propagate through the biliary tree
        self.Cb = dcmri.res_comp(t, p.Kbh*self.Ch, np.divide(1,p.Kgb))
        # Return R1
        return p.R10 + self.aorta.rp*self.Ce + self.rh*self.Ch + self.rh*self.Cb
    ...
    def parameters(self):
        return [
            ...
            ['Kbh', "Biliary tissue uptake rate", 1/(20*60), 'mL/sec/mL', 0, np.inf, True, 6],
            ['Dh', "Hepatocellular dispersion", 90, '%', 0, 100, True, 6],
            ['Kgb', "Biliary tissue excretion rate", 1/(40*60), 'mL/sec/mL', 0, np.inf, True, 6],
        ]
    ...
    def set_export_pars(self):
        ...
        self.export_pars.loc['Kgb', ['value', 'unit']] = [6000*p.Kgb, 'mL/min/100mL']
        ...
        self.export_pars.loc['Tb'] = ["Biliary transit time", np.divide(1, p.Kgb)/60, 'min']
        ...
        self.export_pars.loc['khe'] = ["Hepatocellular uptake rate", 6000*p.Khe*np.divide(p.fb, p.Kve), 'mL/min/100mL'] # Khe Ce = Khe ve ce = khe ce
    ...
    def plot_with_conc(self, fit=True, xrange=None, win='all', show=True, save=False, path=None, legend=True, prefix=''):
        ...
        ax2.plot(self.aorta.t, self.Cb, 'g-.', label='Biliary')
        ax2.plot(self.aorta.t, self.Ce+self.Ch+self.Cb, 'b-', label='Tissue')
        if legend:
            ax2.legend()
        if save:   
