...
class Model(lib.FitFunc):
    ...
    def export_pars(self, export_pars):
        ...
        # Add derived
        F_k = (p.F_ki + p.F_kf)/2
        export_pars.loc['Tb'] = ["Blood mean transit time", np.divide(1, p.Kvp), 'sec']
        export_pars.loc['F_k'] = ["Filtration fraction", 100*F_k, '%']
        export_pars.loc['E_k'] = ["Extraction fraction", 100*np.divide(F_k, 1+F_k), '%']
        return export_pars
