...
class OneShotOneScan(CurveFit):
    ...
    def parameters(self):
        ...
        return [
            ...
            ['Ktrans1', "Hepatocellular uptake rate", self.Ktrans, 'mL/sec/mL', 0, np.inf, True, 7],
            ['Ktrans2', "Hepatocellular uptake rate", self.Ktrans, 'mL/sec/mL', 0, np.inf, True, 7],
            ['Ktrans3', "Hepatocellular uptake rate", self.Ktrans, 'mL/sec/mL', 0, np.inf, True, 7],
    ...
    def set_export_pars(self):
        ...
        self.export_pars.at['Ktrans1','unit'] = 'mL/min/mL'
        self.export_pars.at['Ktrans1','value'] = 60*p.Ktrans1
        self.export_pars.at['Ktrans2','unit'] = 'mL/min/mL'
        self.export_pars.at['Ktrans2','value'] = 60*p.Ktrans2
        self.export_pars.at['Ktrans3','unit'] = 'mL/min/mL'
        self.export_pars.at['Ktrans3','value'] = 60*p.Ktrans3   
        self.export_pars.at['Th','unit'] = 'min'
        self.export_pars.at['Th','value'] = p.Th/60  
        # Add derived parameters   
        self.export_pars.loc["Ktrans"] = ["Hepatocellular uptake rate", 60*(p.Ktrans1+p.Ktrans2+p.Ktrans3)/3, 'mL/min/mL']
    ...
    def R1(self):
        ...
        ne, nh, Ktrans = dcmri.residue_high_flow_2cfm_varK(self.aorta.t, cp, p.Ktrans1, p.Ktrans2, p.Ktrans3, p.Th, p.FpTe)

        self.ce = (1-Ktrans/self.Fp)*cp
        self.ch = nh/self.vh
        self.cl = ne + nh
...
class TwoShotTwoScan(OneShotOneScan):
    ...
    def estimate_p(self):
        ...
        n0 = math.floor(60/self.aorta.tacq) # 1 minute baseline harwired!!
...
class OneShotTwoScan(TwoShotTwoScan):
    ...
    def plot_fit(self, show=True, save=False, path=None, prefix=''):
        ...
        self.plot_with_conc(win='acq2', xrange=[t0, t1], show=show, save=save, path=path, prefix=prefix)
...
class TwoShotOneScan(OneShotOneScan):
    ...
    def plot_fit(self, show=True, save=False, path=None, prefix=''):
        ...
        self.plot_with_conc(show=show, save=save, path=path, prefix=prefix)
        BAT2 = self.aorta.p.value.BAT2
        self.plot_with_conc(win='shot2', xrange=[BAT2-20, BAT2+160], show=show, save=save, path=path, prefix=prefix)
        self.plot_with_conc(win='shot2_', xrange=[BAT2-20, BAT2+600], show=show, save=save, path=path,prefix=prefix)
        self.plot_with_conc(win='shot2__', xrange=[BAT2-20, BAT2+1200], show=show, save=save, path=path, prefix=prefix)
