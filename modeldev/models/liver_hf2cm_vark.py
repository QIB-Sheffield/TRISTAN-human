...
class TwoShotTwoScan(CurveFit):
    ...
    def R1(self): # slower but more diagnostics
        ...
        self.Ch = dcmri.res_comp(self.aorta.t, k_he*ca, p.Th)
        # Return R1
    ...
    def parameters(self):
        return [
            ...
            ['S01', "Signal amplitude S0", 1200, "a.u.", 0, np.inf, True, 4],
            ['S02', "Signal amplitude S0", 1200, "a.u.", 0, np.inf, True, 4],
            # Inlets
            ['Tdel', "Gut delay time", 5.0, 'sec', 0, 20.0, True, 3],  
            ['Te', "Extracellular transit time", 30.0, 'sec', 0, 5*60, True, 3],
            # Liver tissue
            ['ve', "Extracellular volume fraction", 0.2, 'mL/mL', 0.0, 1.0, True, 3],
            ['Th', "Hepatocellular transit time", 30*60, 'sec', 10*60, np.inf, True, 6],
    ...
    def set_export_pars(self):
        ...
        self.export_pars.loc['Th', ['value', 'unit']] = [p.Th/60, 'min']
        self.export_pars.loc['k_he_i', ['value', 'unit']] = [6000*p.k_he_i, 'mL/min/100mL']
        self.export_pars.loc['k_he_m', ['value', 'unit']] = [6000*p.k_he_m, 'mL/min/100mL']
